...
**v3.4, the iMessage cut, is built.** Fourteen shots over the vstar track, 119.699 s, cut to the beat grid in `v3/cutsheet.json`. `PLAN-v3.md` is the brief; section 13 records what shipped and section 14 records the v3.1 rebuild (camera, takeovers, Arlan Vault effects) section 15 the v3.2 pass (sound design, the audio-driven grade, the app icon), section 16 the v3.3 notes pass (foley level, no app-count claims, no carousels or orbits, and every phone in the film — the end card's included — lit and mid-conversation), and section 17 the v3.4 component pass (the first half another 6 dB down, and ten React Bits studies re-authored for linear time). `v3/COMPONENTS-v3.md` holds the component rulings. The v2 compositions are gone from the tree (git history keeps them); `PLAN-v2.md` and `STORYBOARD.md` stay as the source of the approved claim copy.
...
- `assets/bgm-mix.mp3` — what the film plays: the vstar track with the 94-cue foley layer mixed under it
