...
box's dashboard hosted route with basic auth from the sealed
`boxes.dashboard_auth` credential (see `SECURITY-DECISIONS.md`). Asset
...
2. Rebuild the template box: run `infra/template/setup.sh` in the template
   (it re-copies the plugin into `~/.hermes/plugins/creative` and records
   `CREATIVE_PLUGIN_VERSION` in `~/.hermes/.env`), then warm and stop it,
