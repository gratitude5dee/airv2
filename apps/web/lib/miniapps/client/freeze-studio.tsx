...
import type { ThreeStage } from "./freeze-three-stage";
...
export function poseAt(frames: CameraKeyframe[], t: number): CameraKeyframe {
...
function StageCanvas(props: StageProps) {
  ...
  useEffect(() => {
    ...
    // `three` (~660 KB) rides an async chunk so first paint doesn't pay for
    // it — the stage area stays empty until the 3D viewport attaches; the
    // flat 2D canvas below only takes over if WebGL or the chunk itself is
    // unavailable.
    ...
    void import("./freeze-three-stage")
      .then(({ createThreeStage }) => {
        if (cancelled) return;
        try {
          stage = createThreeStage(host);
        } catch {
          setGlFailed(true); // no WebGL — 2D canvas keeps editing working
          return;
        }
        stageRef.current = stage;
        propsRef.current.viewCtl.current = ctl;
        // A freshly mounted stage always starts at the home view — clearing
        // the flag here also covers remounts (tab switch), where the previous
  ...
  return (
    ...
    >
      {!glReady && (glFailed || props.lite) && <StageCanvas2D {...props} />}
    </div>
  );
}
