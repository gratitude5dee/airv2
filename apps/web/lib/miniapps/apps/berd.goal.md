...
**3.3 Box-hosted Berd (optional, later).** If a user runs Berd inside their own Box (headless `goosed` + `berdctl`), the control plane already has an authenticated path to the Box and can invoke `berdctl` there through the existing box command lane — no pairing needed, same envelope validation, same allowlist. Treat this as a variant of 3.1's server side, not a second product.
