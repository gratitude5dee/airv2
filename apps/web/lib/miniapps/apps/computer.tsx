...
function renderState(
  ...
  // without it (black screen with working audio/none). Keyboard input goes
  // directly to the focused viewer; fullscreen is the reliable fallback for
  // webviews that do not focus a cross-origin frame correctly.
  const live = embed
    ? `<div style="margin-top:10px;border-radius:var(--radius-well);overflow:hidden;box-shadow:var(--shadow);background:#000"><iframe id="live-desktop" src="${esc(basePath)}?view=live" title="Live view of your agent's computer" allow="autoplay; fullscreen; clipboard-read; clipboard-write" allowfullscreen referrerpolicy="no-referrer" style="display:block;width:100%;height:${lite ? "min(62vh,520px)" : "max(62vh,420px)"};border:0"></iframe></div><p class="muted" style="margin-top:8px;display:flex;align-items:center;gap:0.75rem;flex-wrap:wrap"><span class="chip on">Connecting</span><a href="${esc(basePath)}?view=live" target="_top" style="display:inline-flex;align-items:center;min-height:2.75rem">Open full screen</a><a href="${esc(basePath)}?view=live&amp;vnc=1" target="_blank" rel="noopener" style="display:inline-flex;align-items:center;min-height:2.75rem">Choppy? Switch to VNC</a></p>`
    : "";
  const shot =
