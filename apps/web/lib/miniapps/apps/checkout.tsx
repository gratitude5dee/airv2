...
function renderCheckout(ctx: MiniAppContext, h: CheckoutHandoff): NextResponse {
  ...
  const kernelWatch = h.kernel_session_id
    ? `<p><a href="${esc(mintSignedLink(ctx.session.userId, "watch", "default"))}">Watch the agent's cloud browser</a></p>`
    : "";
  const body = `<section class="panel"><div class="item"><span class="grow"><strong>${esc(h.item_summary)}</strong><br><span class="muted">${esc(h.merchant_host)} · ${esc(h.quantity === null ? "quantity unknown" : String(h.quantity))} · ${esc(amount(h))}</span>${details}</span><span class="when">${esc(status.replaceAll("_", " "))}</span></div>${blocker}${payment}<p><a href="${esc(browserLaunch)}" target="_blank" rel="noopener">Open checkout in browser</a></p><p class="muted">Use this if the Messages sheet does not load. It creates a fresh, short-lived browser session for this handoff.</p><p><a href="${esc(h.merchant_url)}" target="_blank" rel="noopener">Open merchant site — you may need to rebuild the cart</a></p>${kernelWatch}${browser}${status === "expired" ? '<p class="muted">This checkout hold expired. Ask Air for a fresh handoff.</p>' : ""}${status === "completed" ? '<p>Payment completed — verify the merchant receipt before closing this task.</p>' : ""}${cancel}</section>`;
