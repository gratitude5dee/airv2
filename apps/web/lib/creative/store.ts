...
export async function ingestGeneratedMedia(
  ...
  if (!ext) {
    throw new AssetPipelineError("generated media has an unsupported type");
  }
  ...
  const upload = await supabase.storage
    .from(ASSETS_BUCKET)
    .upload(key, media.bytes, { contentType: contentType(ext), upsert: true });
  if (upload.error) {
  ...
  const inserted = await supabase
    ...
    .insert({
      user_id: userId,
      // Not a box-side asset: record the provider origin as the ref.
      box_asset_id: `gmi:${digest}`,
      sha256: digest,
      ext,
      kind: ext,
