...
describe("adminAuditRow", () => {
  it("carries the action, the app ids and a metadata detail only", () => {
    expect(
      adminAuditRow({ action: "dev_renew", app, operator: "carol", detail: { version: "v1700000000001" } })
    ).toEqual({
      actor: "admin",
      action: "dev_renew",
      app_id: "app-1",
    ...
    expect(adminAuditRow({ action: "suspend", app, operator: "carol" }).detail).toEqual({});
  });
});
...
describe("recordAdminAudit", () => {
  it("inserts one admin_audit row", async () => {
    ...
    expect(await recordAdminAudit(db.client(), { action: "dev_revoke", app, operator: "carol" })).toBe(true);
    expect(db.inserts).toHaveLength(1);
    expect(db.inserts[0]).toMatchObject({
      table: "admin_audit",
      row: { action: "dev_revoke", app_id: "app-1", slug: "alice-promo", operator: "carol" },
    });
  });
  ...
  it("logs and returns false when the ledger write fails", async () => {
    ...
    expect(await recordAdminAudit(db.client(), { action: "suspend", app, operator: "carol" })).toBe(false);
