 ...
 * detail }` triple the web Progress pane shows — a model's "I'm 80% done"
 * is text, not state. `readProgress` gathers that state (intake stage,
 * latest build, the draft's version row). V13: the card is sent once by
 * /api/create/go and never edited — the live progress channel is the
 * OwnerRoom DO + create_jobs row, so the flush-side relay is gone.
 *
