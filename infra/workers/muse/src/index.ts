...
${inviteUrl ? `<p class="note">Your new Air is being set up. Its iMessage invite is ready: <a href="${escapeHtml(inviteUrl)}">open Messages</a>. You can also sign back into Air later with this same phone number.</p>` : ""}
...
async function fullAuthorize(request: Request, env: MuseEnv): Promise<Response> {
  ...
  if (stage === "code") {
    ...
    if (outcome.status !== 200 || !outcome.value.user_id) return fullCodePage(parsed, action, phone, randomToken(), outcome.value.error === "no_line_available" ? "Air is temporarily out of iMessage lines. Please try again later." : "That code did not work. Try again.");
