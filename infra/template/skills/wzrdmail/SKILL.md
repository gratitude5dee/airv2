...
metadata:
  hermes:
    tags: [Email, wzrdmail, SDK, Drafts]
---

> Vendored from `plugins/agents/skills/wzrdmail` in gratitude5dee/wzrdmail; installed on
> every Air box by `infra/template/setup.sh` and used when `MAIL_PROVIDER=wzrdmail`.
> On a box the key is `WZRDMAIL_API_KEY` (inbox-scoped, `read,drafts`) and the inbox is
> `WZRDMAIL_INBOX_ID`; use the `wzrdmail` MCP `create_draft` tool — see `email-draft-review`.
