...
| Secondary outcome | A Muse user with **no** Air account gets one from the consent screen: after phone verification, Air reserves an available Photon/Spectrum project line and provisions a phone-bound account, Box and WZRDMail inbox; the user sees that Air line and sends the first iMessage (the existing inbound-first rule), so their Muse plan's tokens do the thinking while Air does the doing |
...
- **GP2 — get an Air account from Muse.** Same flow, phone unknown to Air → the consent page verifies the phone, allocates an available Air-project Photon/Spectrum line, provisions the account, Box and WZRDMail inbox, and shows the `sms:` invite. The owner's first iMessage establishes the conversation (C13); it does not defer the box fork. *Exit:* a box exists; GP1's exit holds.
