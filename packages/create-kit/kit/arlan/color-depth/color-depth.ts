export function initColorDepth(root: ParentNode = document) {
  const reduce = matchMedia("(prefers-reduced-motion: reduce)").matches;

  if (!reduce) {
    root.querySelectorAll<HTMLElement>(".depth-metal, .depth-foil").forEach((el) => {
  ...
  root.querySelectorAll<HTMLElement>(".depth-toggle").forEach((el) => {
